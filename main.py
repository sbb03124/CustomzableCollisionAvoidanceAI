...
def main(visualize, reward_params):
    ...
    agent = model.Agent(
        ...
        memory = model.Memory(maxlen=2000000, window_length=window_length),
...
def debug():
    ...
    agent = model.Agent(
        ...
        memory = model.Memory(maxlen=batch_size*100, window_length=window_length),
        actor_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, decay=0.5*1e-8),
        critic_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, decay=0.5*1e-8),
        batch_size = batch_size,
        gamma = 0.99,
...
def test(weight_file, nb_test, visualize, save=False, params=None):
    ...
    agent = model.Agent(
        ...
        memory = model.Memory(maxlen=window_length, window_length=window_length),
        actor_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, decay=0.5*1e-8),
        critic_optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, decay=0.5*1e-8),
        batch_size = batch_size,
        gamma = 0.99,
...
if __name__=='__main__':
    ...
    if args.test is None:
        ...
        test(args.test, args.epi, args.visualize, save=False, params=_params)